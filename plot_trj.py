...
class cell ():
    ...
    def plot_cell(self, ax , color='gray', legend=''):
        for i in range(len(self.vrt)-1):
            ax.plot([self.vrt[i,0], self.vrt[i+1,0]], [self.vrt[i,1], self.vrt[i+1,1]], color = color)
        
        if len(legend) == 0:
            ax.plot([self.vrt[0,0], self.vrt[-1,0]], [self.vrt[0,1], self.vrt[-1,1]], color = color)
        else:   
            ax.plot([self.vrt[0,0], self.vrt[-1,0]], [self.vrt[0,1], self.vrt[-1,1]], color = color, label=legend)
...
# p4 = np.array([3.4, -4.0])
# p5 = np.array([3.4, 0.5])
p0 = np.array([0.04, 0.35])
p1 = np.array([0.04, -1.35])
p2 = np.array([1.6 ,-1.35])
p3 = np.array([1.6, -3.6])
p4 = np.array([3.4, -3.6])
p5 = np.array([3.4, 0.35])
...
def plot_poses_npy(poses_path='data/poses.npy', ax=None, color='red', trajectory_alpha=0.7,
                   show_heading=True, heading_step=1, heading_scale=0.15, heading_color='darkred'):
    """
    Load poses from a .npy file (x, y, yaw) and plot position trajectory and headings.

    Expects array of shape (N, 3) with columns [x, y, yaw]; yaw in radians.

    Parameters:
    poses_path: Path to .npy file (default: 'data/poses.npy')
    ax: matplotlib axes object to plot on (if None, creates a new figure)
    color: Color for the trajectory line (default: 'red')
    trajectory_alpha: Transparency of the trajectory (default: 0.7)
    show_heading: Whether to draw heading arrows (default: True)
    heading_step: Plot heading arrow every N poses to avoid clutter (default: 1 = every pose)
    heading_scale: Arrow length scale for quiver (default: 0.15)
    heading_color: Color for heading arrows (default: 'darkred')

    Returns:
    poses: numpy array of shape (N, 3) [x, y, yaw], or None if load failed
    ax: matplotlib axes object used for plotting
    """
    poses_path = Path(poses_path)
    if not poses_path.exists():
        print(f"Error: Poses file '{poses_path}' not found.")
        return None, ax

    try:
        poses = np.load(poses_path)
    except Exception as e:
        print(f"Error loading poses from '{poses_path}': {e}")
        return None, ax

    if poses.ndim != 2 or poses.shape[1] < 3:
        print(f"Error: Expected poses shape (N, 3), got {poses.shape}.")
        return None, ax
    frq_sample = 15
    poses = poses[::frq_sample]
    poses = poses[:150]
    x = poses[:, 0]
    y = poses[:, 1]
    yaw = poses[:, 2]
    ...
    if ax is None:
        ...
        ax.set_xlabel('X (m)')
        ax.set_ylabel('Y (m)')
        ax.set_title(f'Pose trajectory and headings ({poses_path.name})')
        ax.grid(True)

    # Trajectory
   
    # Headings (yaw) as arrows: u = cos(yaw), v = sin(yaw)
    if show_heading and len(poses) > 0:
        step = max(1, int(heading_step))
        idx = np.arange(0, len(poses), step)
        if len(idx) == 0:
            idx = np.array([0])
        u = np.cos(yaw[idx])
        v = np.sin(yaw[idx])
        ax.quiver(x[idx], y[idx], u, v, color=heading_color, scale=1.0 / heading_scale,
                  scale_units='xy', angles='xy', width=0.03, alpha=0.8, label='Heading')

    # ax.legend(loc='best')
    ax.plot(x, y, color='green',linewidth=4, linestyle='--', alpha=trajectory_alpha, label=f'Trajectory ({len(poses)} poses)')

    print(f"Plotted {len(poses)} poses from '{poses_path}'.")
    return poses, ax
...
def plot_vector_field(ax, pos_file='pos_ls.npy', control_file='control_ls.npy', 
    ...
    ax.quiver(x_pos, y_pos, ux, uy, angles='xy', scale_units='xy', 
              color=color, alpha=alpha, width=width, label='Control commands')
    ax.plot(x_pos, y_pos, color='green', linewidth=2, alpha=0.7, label='Trajectory')
...
if __name__ == "__main__":
    ...
    ax.set_aspect('equal')
    ax.tick_params(axis='both', labelsize=14)  # larger font for axis numbers
    ...
    c9.plot_cell(ax)
    c10.plot_cell(ax)
    c11.plot_cell(ax, legend= 'Convex regions')
    c_Lshape.plot_cell(ax, color='red', legend= 'L-shape environment')
    # plot_pose_from_bag('test_2026-01-27-15-49-04', ax=ax, color='blue', markersize=3)
    
    # plot_vector_field(ax, pos_file='trj_data_apri8/pos_ls.npy', control_file='trj_data_apri8/control_ls.npy')
    ...
    # plot_poses_npy(poses_path='data/poses.npy', ax=ax, c  olor='red', trajectory_alpha=0.7,
    #                show_heading=True, heading_step=1, heading_scale=0.15, heading_color='blue')
    ax.legend(fontsize=15)
    # fig.savefig('real_robot_trj.png', dpi=600)
    plt.show()